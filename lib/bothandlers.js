...
const addHandlers = function (bot) {
    ...
    bot.socket.on("emoteList", (emotes) => {
        if (!bot.enableWebServer) return;
        bot.channelEmotes = emotes;
    });

    bot.socket.on("error", (err) => bot.logger.errlog.log(err));
    ...
    bot.socket.on("pm", (data) => {
        bot.checkPermission(data["username"], 2, null, (hasPermission) => {
            if (hasPermission) bot.handleChatMsg(data, true);
        });
    });
